...
- **텍스트 (text)**: 실행 코드가 저장되는 영역
- **데이터 (data)**: 전역 변수 및 정적 변수 저장 영역
- **힙 (heap)**: 동적 메모리 할당 영역 (`malloc()` 등)
- **스택 (stack)**: 함수 호출 시 지역 변수 및 반환 주소 저장
- **U-영역 (user area)**: 열린 파일, 현재 작업 디렉터리 등 프로세스 관련 정보 저장

---

### 📝 핵심 요약

- 쉘은 사용자 명령어를 처리하고 프로세스를 생성하는 인터페이스 역할을 수행
- `exec`는 새 프로그램을 실행하고, `exit`는 프로세스를 종료시킴
