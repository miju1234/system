...
- **쉘(Shell)**: 사용자와 운영체제 사이에서 명령어 인터페이스 역할을 수행
- **전면 처리**: 실행이 끝날 때까지 쉘이 대기
- **후면 처리**: `&` 사용 → 명령 실행 중에도 쉘에서 다른 작업 가능  
  예: `sleep 5 &`, `jobs`, `fg %1`
- **프로세스(process)**: 실행 중인 프로그램  
  - `ps`, `ps -ef`, `ps aux` 등으로 상태 확인 가능
...
- **환경 변수**
  - 전체 보기: `environ`, `getenv("KEY")`
  - 설정/삭제: `setenv("KEY", "VALUE", 1)`, `unsetenv("KEY")`, `putenv("KEY=VALUE")`
